  ...
  return (
    <main className="py-12 md:py-16 lg:py-20">
      <div className="mx-auto max-w-6xl px-4 md:px-8">
        <div className="motion-rise mb-8 flex flex-col gap-3">
          <p className="luxury-kicker">Cart</p>
          <h1 className="font-heading text-3xl md:text-4xl">Your Bag</h1>
        </div>
        <div className="grid gap-8 lg:grid-cols-[2fr_1fr]">
          <div className="motion-rise motion-rise-delay-1 space-y-4">
            {items.map((item) => (
              <CartItem key={item.key} item={item} />
            ))}
          </div>
          <div className="luxury-surface motion-rise motion-rise-delay-2 h-fit rounded-3xl p-6">
            ...
            <Link
              href="/products"
              className="mt-4 inline-flex w-full items-center justify-center rounded-full border border-black/10 bg-white px-4 py-3 text-xs uppercase tracking-[0.2em] text-ink transition-colors duration-300 hover:border-black/30"
            >
              Continue Shopping
            </Link>
          </div>
